...
- **modules/ServiceManagement.psm1** – installs/updates/starts/stops the Windows service, exposes status queries and monitor mode. `Start-ReAIService` reinstalls the service if it is missing or misconfigured, logs can be viewed with `View-ReAILog`, and `Register-ReAIStartup` creates a log-view task at user logon so a terminal stays open whenever the machine is online.
- **modules/StateManagement.psm1** – provides `Save-State` for persisting the global state object.
- **modules/OpenAIUtils.psm1** – OpenAI helpers plus DuckDuckGo (Tor), Google, Google Scholar and arXiv search utilities used by multiple modules. When Tor mode is requested the module now tries `curl.exe` first and falls back to `Invoke-RestMethod` if unavailable. Rate limiting defaults to 30 seconds between requests (2 per minute) when no settings are provided.
...
- Modules are imported using `-DisableNameChecking` to suppress warnings about unapproved verbs.
- **modules/ResearchSummary.psm1** – generates lab reports, creative articles and business plans from a topic using API utilities and self-refactor. It aggregates reliability ratings to produce an overall score summary and saves a Google-style overview of sources. Invoked with the `-ResearchTopic` CLI switch.
- **modules/BusinessPlanning.psm1** – builds comprehensive business plans including executive summary, SWOT analysis, market strategy, operations, financial projections, risk assessment and timeline. Called by `ResearchSummary.psm1`.
- **modules/BusinessAdvisory.psm1** – crafts ongoing strategic guidance for generated business plans, outlining action items, growth opportunities, risk monitoring, key metrics and next steps. Called by `ResearchSummary.psm1`.
- **modules/ResearchManagement.psm1** – scans saved research summaries, writes an aggregated `compressed_context.txt` for ChatGPT prompts, and uses GPT to suggest new areas of study, automatically adding them as goals. Called when `-ManageResearch` is used or from the menu.
- **modules/ResearchDatabase.psm1** – lists stored research topics and shows detailed source data saved as `*_sources.json`. Invoked via `-ListResearch`, `-ShowResearch` and menu options.
...
- **modules/PromptDefense.psm1** – detects prompt injection attempts, validates GPT responses, verifies terminal sessions and warns if ReAI runs from an unexpected path. Called by `Chatbot.psm1`, `OpenAIUtils.psm1` and `ReAI.ps1`.
- **modules/Logging.psm1** – provides `Write-ReAILog` for standardized log output to `reai.log`.
- **modules/ErrorHandling.psm1** – offers `Invoke-ReAISafely` and `Set-ReAIErrorHandler` for centralized exception logging.
...
- **modules/ReahModel.psm1** – maintains a trigram Markov chain built from corpus text and past chats, retraining as new lines are added for more coherent replies.
- **modules/Chatbot.psm1** – provides local or GPT-based conversation, updating the model after each exchange and saving transcripts to `chat_logs/`. Commands prefixed with `!` (e.g., `!listgoals`, `!research`) invoke other module functions. Invoked via the `-Chat` or `-ChatGPT` flags and menu.
- **modules/MoodManager.psm1** – stores Reah's conversational mood (`friendly` or `functional`) and returns mood-specific GPT prompts. `Chatbot.psm1` calls `Get-MoodSystemPrompt` and exposes a `!mood` chat command to switch modes.
...
- New logging module standardizes log output across commands.
  Log writes now use a shared file stream with retry logic so multiple processes can append without file-lock errors; the previous Start-Transcript usage was removed to prevent locking the log file.
...
- ResearchManagement now aggregates summaries into `reports/compressed_context.txt` for condensed prompts.
...
- Added install_and_start.ps1 script for a single-step installation and service startup. It now tails the log within the same terminal after setup using `-ViewLog`.
...
- ResearchSummary now saves per-source details to `*_sources.json`. ResearchDatabase module exposes `List-ResearchTopics` and `Show-ResearchSources` via `-ListResearch`, `-ShowResearch` and menu options.
...
- Created `notes/recommendations.md` to track future improvements and added corresponding tasks to `goals.md`.
- Began expanding test coverage with a Pester test for the Logging module.
- setup.ps1 now logs module installation failures and continues so ReAI can start in limited mode. install_and_start.ps1 falls back to launching ReAI directly if the service fails to install, and codex_setup.sh proceeds even when package installs fail.
- install_and_start.ps1 registers a user-logon startup entry so the log-view terminal opens automatically, keeping a terminal available whenever the PC is online.
