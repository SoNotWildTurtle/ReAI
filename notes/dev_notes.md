...
- **modules/ServiceManagement.psm1** – installs/updates/starts/stops the Windows service, exposes status queries and monitor mode. `Start-ReAIService` reinstalls the service if it is missing or misconfigured and logs can be viewed with `View-ReAILog`.
