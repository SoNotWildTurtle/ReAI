...
## Setup
1. Run `scripts/install_and_start.ps1` to install dependencies, configure tokens, install the service and open a persistent log terminal automatically.
...
- `-ServiceStatus` – show current service status
 ...
 - `-OpenTerminal` – open a separate terminal to tail the log file
 - `-CloseTerminal` – close the log terminal if running
 - `-Monitor` – monitor the service and restart it automatically if it stops; reopens the log terminal if necessary
...
| `-OpenTerminal` | Open a separate log-view terminal |
| `-CloseTerminal` | Close the log-view terminal |
...
| `ServiceManagement.psm1` | Start/stop/restart the Windows service (Windows 10+ only), open/close a log terminal, check status and monitor the service. | `ReAI` CLI | Windows `Start-Service`, `Stop-Service`, `Get-Service`, `Start-Process` |
