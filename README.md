...
## Setup
1. Run `scripts/install_and_start.ps1` to install dependencies, configure tokens, install the service and display the log within the terminal automatically.
...
| `ServiceManagement.psm1` | Install/update/start/stop/restart the Windows service (Windows 10+ only), view the log, check status and monitor the service. | `ReAI` CLI | Windows `New-Service`, `Start-Service`, `Stop-Service`, `Get-Service` |
