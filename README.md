...
## Setup
1. Run `scripts/install_and_start.ps1` to install dependencies, configure tokens, install the service and display the log within the terminal automatically. If the service installation fails, the script falls back to launching `ReAI.ps1` directly.
...
4. Alternatively on Codex or other Linux environments run `scripts/codex_setup.sh` to install PowerShell and project dependencies. The script continues even if package installation fails.
5. On Windows you may run `scripts/setup.ps1` directly to install required PowerShell modules and create runtime directories. Missing modules are logged as warnings so ReAI can still start with limited features.
...
9. Optionally set `OPENAI_MAX_RPM` to your OpenAI account's request-per-minute limit. The script waits `240 / OPENAI_MAX_RPM` seconds between calls (or 60 seconds for `gpt-4o`). If no rate is provided, ReAI defaults to 2 requests per minute (30 seconds between calls). You can override the interval directly using `OPENAI_RATE_LIMIT`.
10. GPT responses are cached under `cache/gpt_cache.json` to inform future calls, but every request still goes to the API.
11. Execute `./ReAI.ps1 -InstallService` to install the Windows service (Windows 10+ only).
   The service starts `ReAI.ps1` with no arguments so the interactive menu appears in its terminal window. `-StartService` automatically installs or reinstalls the service if it is missing or misconfigured. On non-Windows systems use the script directly instead of the service.

## Runtime Controls
- `-InstallService` – install or update the Windows service (Windows 10+ only)
...
- `-ViewLog` – display the log file within the current terminal
- `-Monitor` – monitor the service and restart it automatically if it stops
- `-RegisterStartup` / `-UnregisterStartup` – add or remove a log-view task that launches at user login (Windows 10+ only)
...
- `-ResearchTopic "subject"` – generate a research report and comprehensive business plan (SWOT analysis, timeline)
- `-ProcessGoal "task"` – run goal processing workflow for a specific goal
- `-ProcessAllGoals` – process each active goal sequentially
- `-ManageResearch` – aggregate existing research and suggest new goal topics
- `-ListResearch` – list topics with saved research sources
- `-ShowResearch "topic"` – display saved source details for a topic
...
- `-ExportConfig <file>` - export current configuration to a JSON file
- `-ImportConfig <file>` - import configuration from a file
...
- `-ConfigureTokens` – interactively set environment variables like API keys
 - `-Chat` – open an interactive chatbot session powered by the local Reah model; prefix commands with `!` (e.g., `!listgoals`, `!research <topic>`)
 - `-ChatGPT` – chat using OpenAI responses instead of the local model; `!` commands are still available
...
| Command | Description |
|---------|-------------|
| `-InstallService` | Install or update the Windows 10+ service |
...
| `-ViewLog` | Display the log file in the current terminal |
| `-Monitor` | Restart the service automatically if it stops |
| `-RegisterStartup` / `-UnregisterStartup` | Add or remove a log-view task that launches at user login |
...
| `-ResearchTopic` | Produce a research report and comprehensive business plan |
| `-ProcessGoal` / `-ProcessAllGoals` | Run the research pipeline for one or all goals |
| `-ManageResearch` | Aggregate prior research and suggest new goals |
| `-ListResearch` | List topics with saved research sources |
| `-ShowResearch` | Display saved source details for a topic |
| `-EnableSecureMode` / `-DisableSecureMode` | Toggle network blocking secure mode |
...
| `-ExportConfig` | Export current configuration to a file |
| `-ImportConfig` | Import configuration from a file |
...
You can also chat with Reah in a loop that learns from previous conversations to produce increasingly unique replies. Prefix lines with `!` to run internal commands such as `!listgoals` or `!research <topic>` while chatting. Use `!mood friendly` or `!mood functional` to switch between her playful personality and a concise, task-focused mode.

During research, each information source is rated for reliability using GPT to help prioritize trustworthy data. The research summary includes an average reliability score, a breakdown of high, medium and low rated sources, and a Google-style overview listing each source with its snippet and score. Source details are also saved to `*_sources.json` for later review via `-ListResearch` and `-ShowResearch`. Self-evolution now runs the full test suite before and after refactoring and logs whether the mutation is beneficial or needs review.
Compressed summaries of generated reports are stored as `*_summary.txt` next to the full encrypted files so Reah can recall key points without loading the entire document. A Google-style overview of top sources is written to `*_google_summary.md`. A combined `compressed_context.txt` aggregates all summaries for minimal-context prompts.
GPT requests are throttled automatically. `OPENAI_MAX_RPM` determines the wait time (`240 / OPENAI_MAX_RPM` seconds, or 60s for `gpt-4o`). If no limit is set during setup, ReAI assumes 2 requests per minute (30 seconds between calls). `OPENAI_RATE_LIMIT` overrides with an explicit interval. When the API key is missing or network access is disabled, context and text summaries use local fallback processing instead of GPT.

All state is stored in `state.json` in the project directory.
The file is secured with restrictive permissions when first loaded. Prompts are checked for injection patterns, API responses are validated to detect spoofing, and the startup routine warns if the script is executed from an unexpected path.
...
- `scripts/` – helper utilities such as `setup.ps1` for Windows, `codex_setup.sh` for Linux, and `install_and_start.ps1` and `windows_pipeline.ps1` for one-step installation.
 - `reports/` – generated goal reports, business summaries and per-topic source metadata (`*_sources.json`); `compressed_context.txt` holds the aggregated research context.
...
- `cache/` – stored GPT responses to help inform future requests.
...
| `ServiceManagement.psm1` | Install/update/start/stop/restart the Windows service (Windows 10+ only), view the log, register or remove a startup log task, check status and monitor the service. | `ReAI` CLI | Windows `New-Service`, `Start-Service`, `Stop-Service`, `Get-Service` |
...
| `ResearchSummary.psm1` | Generate lab reports, creative articles, detailed business plans and follow-up advisory reports from a topic, store a compressed summary alongside, compute overall reliability metrics, and save a Google-style source overview. Uses search results from multiple engines including Google Scholar and arXiv. | `ReAI` when `-ResearchTopic` is used | `Invoke-GPT`, `Search-Web`, `Get-UrlSummary`, `Save-CompressedText`, `Generate-BusinessPlan`, `Generate-BusinessAdvisory`, `Update-ScriptCode` |
| `BusinessPlanning.psm1` | Build comprehensive business plans with executive summary, SWOT analysis, market strategy, operations, financial projections, risk assessment and timeline. | `ResearchSummary.psm1` | `Invoke-GPT` |
| `BusinessAdvisory.psm1` | Provide ongoing strategic guidance for a business plan with action items, opportunities, risk monitoring, key metrics and next steps. | `ResearchSummary.psm1` | `Invoke-GPT` |
| `ResearchManagement.psm1` | Aggregate past research summaries, maintain a combined `compressed_context.txt`, and recommend new goal topics. | `ReAI` when `-ManageResearch` is used or from menu | `Invoke-GPT`, `Add-ReAIGoal`, `Save-CompressedText` |
| `ResearchDatabase.psm1` | List stored research topics and show detailed source data. | `ReAI` when `-ListResearch` or `-ShowResearch` is used or from menu | none |
...
| `PromptDefense.psm1` | Detect prompt injection, validate API responses, verify terminal sessions and warn if ReAI runs from an unexpected path. | `Chatbot.psm1`, `OpenAIUtils.psm1`, `ReAI.ps1` | none |
| `Logging.psm1` | Logging helper appending entries with shared handle and retry logic to avoid file-lock conflicts. | used throughout modules | `.NET FileStream` |
| `ErrorHandling.psm1` | Centralized helpers to log and handle errors via `Invoke-ReAISafely`. | `ReAI` and modules | `Write-ReAILog` |
...
| `WindowsPipeline.psm1` | Runs the AutoPipeline and ensures the Windows service is running. | `ReAI` when `-WinPipeline` is used | `Invoke-AutoPipeline`, `Start-Service` |
...
| `ReahModel.psm1` | Builds a trigram Markov chain model from corpus text and chat history for more coherent replies. | `Chatbot.psm1` | `Get-Content`, `Add-Content` |
| `Chatbot.psm1` | Interactive conversation using the enhanced local model or GPT with transcripts saved. Supports `!` commands to trigger other features (e.g., `!listgoals`) and mood switching. | `ReAI` when `-Chat` or `-ChatGPT` is used or from menu | `ReahModel.psm1`, `Invoke-GPT`, `MoodManager.psm1` |
| `MoodManager.psm1` | Track and switch Reah's conversational mood (`friendly` or `functional`) and supply GPT system prompts. | `Chatbot.psm1` | `Write-ReAILog` |
...
| `GPTCache.psm1` | Stores GPT request/response pairs so prior answers can guide later requests without reusing them. | `OpenAIUtils.psm1` | `ConvertTo-Json`, `Get-FileHash` |
| `Help.psm1` | Display usage information for CLI and menu. | `ReAI` and menu | none |
| `Configuration.psm1` | Export and import configuration including state and environment variables. | `ReAI` CLI | `Save-State` |
