...
if (-not $env:REAI_ROOT) { $env:REAI_ROOT = $global:WorkDir }
...
function Import-AllModules {
    ...
    $error   = $mods | Where-Object { $_.Name -eq 'ErrorHandling.psm1' }
    if ($logging) {
        Import-Module $logging.FullName -Force -DisableNameChecking
        Write-Host "Loaded module: $($logging.Name)"
    }
    if ($error) {
        Import-Module $error.FullName -Force -DisableNameChecking
        if (Get-Command Write-ReAILog -ErrorAction SilentlyContinue) {
            Write-ReAILog -Message "Module loaded: $($error.Name)" -Level 'INFO'
        }
    }
    foreach ($mod in $mods) {
        if ($logging -and $mod.FullName -eq $logging.FullName) { continue }
        if ($error -and $mod.FullName -eq $error.FullName) { continue }
        try {
            Import-Module $mod.FullName -Force -DisableNameChecking
            Write-ReAILog -Message "Module loaded: $($mod.Name)" -Level 'INFO'
        } catch {
            if (Get-Command Write-ReAILog -ErrorAction SilentlyContinue) {
                Write-ReAILog -Message "Module failed to load: $($mod.Name)" -Level 'ERROR'
            } else {
                Write-Warning "Module failed to load: $($mod.Name)"
            }
        }
    }
}
...
Import-AllModules
if (Get-Command Set-ReAIErrorHandler -ErrorAction SilentlyContinue) {
    Set-ReAIErrorHandler
}
if (Get-Command Setup-ReAIEnvironment -ErrorAction SilentlyContinue) {
    Invoke-ReAISafely -ScriptBlock { Setup-ReAIEnvironment } -Action 'environment setup'
}
if (Get-Command Initialize-Security -ErrorAction SilentlyContinue) {
    Invoke-ReAISafely -ScriptBlock { Initialize-Security } -Action 'security initialization'
} elseif (Test-Path (Join-Path $global:ModulesDir 'IntegrityCheck.psm1')) {
    try { Test-Integrity | Out-Null } catch {}
}
if (Get-Command Check-AntiMirror -ErrorAction SilentlyContinue) { Check-AntiMirror }
...
if ($InstallService) {
    Write-ReAILog -Message 'InstallService requested';
    Install-ReAIService; Start-ReAIService; return
}
...
if ($RegisterStartup) { Write-ReAILog -Message 'RegisterStartup requested'; Register-ReAIStartup; return }
if ($UnregisterStartup) { Write-ReAILog -Message 'UnregisterStartup requested'; Unregister-ReAIStartup; return }
...
if ($ManageResearch) { Write-ReAILog -Message 'ManageResearch invoked'; Manage-Research; return }
if ($ListResearch) { Write-ReAILog -Message 'ListResearch invoked'; List-ResearchTopics; return }
if ($ShowResearch) { Write-ReAILog -Message "ShowResearch: $ShowResearch"; Show-ResearchSources -Topic $ShowResearch; return }
...
if (-not $PSBoundParameters.Count) {
    if (Get-Command Prompt-EnvVariables -ErrorAction SilentlyContinue) {
        Prompt-EnvVariables
    }
    if (Get-Command Show-ReAIMenu -ErrorAction SilentlyContinue) {
        Show-ReAIMenu
    } else {
        Write-Host 'Modules failed to load. Run with -Help for CLI options.' -ForegroundColor Yellow
    }
}
